...
<div class="modal fade" id="admin_profile" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <main id="main" class="main">
    ...
    <section class="section dashboard">
        <div class="container">
                <div class="col-sm-12">
                    <form action="recieved_mod.php" method="POST">
                    <div class="input-group border border-success rounded">
                        <input type="text" class="form-control" placeholder="Enter ID Number here.." required name="sid_num">  

                        <span class="input-group-text"><button name="search" class="btn btn-success" 
        ...
        <div class="container mt-5">
            ...
            <?php 
                ...
                if(isset($_POST['search'])){
                    ...
                    //validate student
                    $val = mysqli_query($conn, "SELECT * FROM students WHERE school_id='$sid'");
                    $val_num = mysqli_num_rows($val);

                      if($val_num <=0 ){
                        ?>  
                          <script>
                              alert("Student ID number Doesn't Exsit!");
                              window.location.href="recieved_mod.php";
                          </script>
                      <?php
                      }else{
                                //validate student in get modules
                                $val2 = mysqli_query($conn, "SELECT * FROM modules_to_get WHERE stud_id_num='$sid'");
                                $val_num2 = mysqli_num_rows($val2);

                                      if($val_num2 <= 0){
                                        ?>  
                                        <script>
                                            alert("Student Modules Doesn't Set!");
                                            window.location.href="recieved_mod.php";
                                        </script>
                                       <?php
                                        ...
                                        $get_stud_det = mysqli_query($conn, "SELECT * FROM students WHERE school_id='$sid'");
                                        while($row_stud = mysqli_fetch_object($get_stud_det)){
                                         
                                          $fn = $row_stud -> fn;
                                          $ln = $row_stud -> ln;
                                          $pic = $row_stud -> profile;
                                          $yr = $row_stud -> yr_level;
                                        }
                                            ...
                                            <div class="row">
                                              ...
                                              <div class="col-6 col-md-4">
                                                <div class="card  border border-success" style="width: 18rem;">
                                                  <img src="../upload/<?php echo $pic;?>" class="card-img-top" alt="...">
                                                  <div class="card-body">
                                                    <h5 class="card-title"><?php echo $fn.' '.$ln;?></h5>
                                                    <span class="card-title">BSIT: <?php echo $yr;?></span>
                                                  </div>
                                                </div>
                                              </div>
                                              ...
                                              <form action="adminprocess.php" method="POST">
                                                      ...
                                                      <thead>
                                                        <tr>
                                                          ...
                                                          <th scope="col"><input type="submit" class="btn btn-success btn-sm" name="stud_mod_save" value="SAVE"></th>
                                                        </tr>
                                                      </thead>

                                                      <tbody>
                                                          <?php
                                                            $get_stud_modules = mysqli_query($conn, "SELECT * FROM modules_to_get WHERE stud_id_num='$sid'");
                                                            while($row_mod = mysqli_fetch_array($get_stud_modules)){
                                                             
                                                            
                                                           ?>
                                                        <tr>

                                                          <td><?php echo $row_mod['subject_code'];?></td>

                                                          <?php 
                                                            $mystat = $row_mod['status'];
                                                            if($mystat == "Pending"){
                                                              ?>
                                                               <td><span class="text-danger"><?php echo $row_mod['status'];?></span></td>
                                                               <td>  <input class="form-check-input border border-success" name="sub[]"
                                                          type="checkbox" value="<?php echo $row_mod['subject_code'];?>" id="flexCheckDefault"></td>
                                                              <?php

                                                            }else{
                                                              ?>
                                                              <td><span class="text-success"><?php echo $row_mod['status'];?></span></td>
                                                              <td> </td>
                                                             <?php
                                                            }
                                                          ...
                                                          ?>
                                                            ...
                                                            <input type="hidden" name="admin_name" value="<?php echo $admin; ?>">
